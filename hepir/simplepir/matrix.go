...
func (m *Matrix) Size() uint64 {
	return m.Rows * m.Cols * 4 // logq = 32, 32bits = 4 B
}
...
// var seed128 [16]byte
// _, err := crand.Read(seed128[:])
func MatrixRand(rows uint64, cols uint64, logmod uint64, mod uint64, seed128 [16]byte) *Matrix {
	...
	seed := int64(binary.LittleEndian.Uint64(seed128[0:8]) ^ binary.LittleEndian.Uint64(seed128[8:16]))
	rng := mrand.New(mrand.NewSource(seed))

	for i := 0; i < len(out.Data); i++ {
		value := uint64(rng.Int63()) % m
		out.Data[i] = C.Elem(value)
	}
