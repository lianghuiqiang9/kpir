...
// keys and vals is sorted
func BuildBinaryFuseBucket[T Unsigned](b *BinaryFuseBuilder, keys []uint64, vals []uint64, isSort bool) (BinaryFuse[T], error) {

	Map := map[uint64][]uint64{}
	if !isSort {
		Map = MakeMap(keys, vals)
	}

	size := uint32(len(keys))
	var filter BinaryFuse[T]
	w := uint32((len(vals) + len(keys) - 1) / len(keys))
	filter.initializeKVSParameters(b, size, w)
	rngcounter := uint64(1)
	filter.Seed = splitmix64(&rngcounter)
	capacity := uint32(len(filter.Fingerprints))
	...
	for {
		...
		for _, key := range keys {
			hash := key //mixsplit(key, filter.Seed)
			//fmt.Println("key: ", key, "hash: ", hash, " invhash", mixsplitInverse(hash, filter.Seed))
	...
	for i := int(size - 1); i >= 0; i-- {
		...
		// The keys have some order, so it is fast beyond our expectation
		if !isSort {
			v = Map[k]
		} else {
			v, _ = GetValInterpolation(keys, vals, int(w), k)
		}
		//fmt.Println("key: ", k, " val: ", v)

		xor2 := KVSFingerPrint[T](k, v)
		//fmt.Println("key: ", k, " val: ", v, " xor2: ", xor2)
		index1, index2, index3 := filter.getHashFromHash(hash)
		found := reverseH[i]
		h012[0] = index1
		h012[1] = index2
		...
		i0, i1, i2 := h012[found]*w, h012[found+1]*w, h012[found+2]*w
		f := filter.Fingerprints
		for k, x := range xor2[:w] {
			f[i0+uint32(k)] = x ^ f[i1+uint32(k)] ^ f[i2+uint32(k)]
		}
...
/*
func BuildBinaryFuseBucketFromMap[T Unsigned](b *BinaryFuseBuilder, keys []uint64, vals []uint64) (BinaryFuse[T], error) {

		//keys, vals = Sort(keys, vals)
		Map := MakeMap(keys, vals)
		//fmt.Println(keys[:10], vals[:10])
		...
		for i := int(size - 1); i >= 0; i-- {
			...
			// The keys have some order, so it is fast beyond our expectation
			//v, _ := GetValBinary(keys, vals, int(w), k)
			v := Map[k]
...
func (filter *BinaryFuse[T]) initializeParameters(b *BinaryFuseBuilder, size uint32) {
	...
	filter.Fingerprints = reuseBuffer[uint64](&b.fingerprints, int(arrayLength))
}

func (filter *BinaryFuse[T]) initializeKVSParameters(b *BinaryFuseBuilder, size uint32, uint64PerVal uint32) {
	arity := uint32(3)
	filter.SegmentLength = calculateSegmentLength(arity, size)
	if filter.SegmentLength > 262144 {
		filter.SegmentLength = 262144
	...
	//filter.Uint64PerVal = uint64PerVal
	filter.ArrayLength = arrayLength
	filter.Fingerprints = reuseBuffer[uint64](&b.fingerprints, int(uint64PerVal*arrayLength))
}
...
/*
func (filter *BinaryFuse[T]) GetHashFromHash(hash uint64) (uint32, uint32, uint32) {
	return filter.getHashFromHash(hash)
}
*/
